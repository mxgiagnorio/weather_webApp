...
export default function App() {
  ...
  function onClose(id){
    setCiudades(ciudadesMostradasEnLaApp => ciudadesMostradasEnLaApp.filter((el) => el.id !==id))
    }  
  ...
  function onSearch (newCity){
    ...
    if(ciudades.length >3){
      alert("No se pueden agregar mas ciudades")
        ...
        fetch(`https://api.openweathermap.org/data/2.5/weather?q=${newCity}&appid=${process.env.REACT_APP_APIKEY}&units=metric`)
        .then(r => r.json())
        .then((weather) => {
      if(weather.main){ 
          const newCity = {
    ...
    <div className={styles.container}>
        ...
        <span style={{ 
         ...
         }> Todavia no has buscado ninguna ciudad</span>
          )}   
